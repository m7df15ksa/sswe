...
require("./niro_servers.js");

const niro = new Client({
  disableMentions: "all"
});
...
niro.on("error", console.warn);
niro.on("ready", () => {
  console.log(`[READY] ${niro.user.tag} has been successfully booted up!`);
  niro.user.setActivity(`MUSIC`, { type: "LISTENING" });
});

setInterval(() => {
  const channel = niro.channels.cache.get(`${process.env.CHANNEL_ID}`);
...
niro.on("message", async message => {
  ...
  if (command === "help" || command === "cmd") {
    const helpembed = new MessageEmbed()
      .setColor("BLUE")
      .setAuthor(niro.user.tag, niro.user.displayAvatarURL())
      .setDescription(
        `
__**Command list:**__
...
> \`invite/inv\` - \`invite link for the bot\``
      ...
      .setFooter(
        "©️ niro",
        "https://cdn.discordapp.com/attachments/720296592254435438/783437611774771220/743b4044977a8292521e94211ee5208a.gif"
      );
...
niro.login(process.env.BOT_TOKEN);
